...
export default class AppBuild extends BaseCommand {
  ...
  public async run() {
    ...
    await fs.move(path.resolve('./front/dist/assets'), targetDir);

    await fs.move(path.resolve('./front/dist/manifest.json'), path.resolve('./public/assets/manifest.json'));

    await execaUtils.execute('npm', ['run', 'build:adonis']);

    await fs.remove(targetDir);
    await fs.remove(path.resolve('./front/dist'));
  }
}
